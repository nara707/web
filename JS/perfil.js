(function() {
    // ── Estrellas (código existente, se mantiene) ──
    ...
    // ----- CARGAR PUBLICACIONES PENDIENTES DESDE LOCALSTORAGE (insertar al inicio) -----
    const pendingPosts = JSON.parse(localStorage.getItem('pendingPosts') || '[]');
    if (pendingPosts.length > 0) {
        const newCards = pendingPosts.map(post => ({
            title: post.title,
            artist: post.artist,
            category: post.category || 'otro',
            price: post.price || '$0',
            catLabel: post.catLabel || post.category || 'Otro',
            date: post.date || new Date().toISOString().slice(0,10),
            likes: post.likes || '0',
            image: post.image
        }));
        // Insertar al principio del array (las nuevas publicaciones aparecen primero)
        allCards.unshift(...newCards);
        // Limpiar localStorage para no volver a cargar los mismos posts
    ...
    function renderGrid() {
        ...
        filtered.forEach(card => {
            const backgroundStyle = card.image 
            ...
            html += `
                <div class="art-card" data-category="${card.category}">
                    <div class="art-card-ph" style="${backgroundStyle}"></div>
    ...
    // ----- MANEJADORES DE SUBFILTER CHIPS -----
    subfilterChips.forEach(chip => {
        chip.addEventListener('click', function(e) {
    ...
    function setActiveTab(tab) {
        ...
        else if (tab === tabSells) currentTab = 'sells';
